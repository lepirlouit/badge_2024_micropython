...
import logging
logger = logging.Logger('rtttl_player')
...
class RTTTL:
    ...
    def notes(self):
        ...
        while True:
            ...
            freq = _NOTE[note_idx] * (1 << (octave - 4))
...
def _play_tone(freq, msec):
    logger.debug('freq = {:6.1f} msec = {:6.1f}'.format(freq, msec))
    if freq > 0:
        buzzer.freq( int(freq) )
        buzzer.duty( int(_DUTY) )
...
def _play(tune):
    print(f"playing '{tune.name}'")
    try:
        for freq, msec in tune.notes():
            _play_tone(freq, msec)
    finally:
        _play_tone(0, 0)


def play_rtttl(rtttl_data):
    _play(RTTTL(rtttl_data))
